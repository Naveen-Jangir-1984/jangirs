    ...
    <div className="filter">
      ...
      <div className="filter-men">
        <select name="village" value={male.village} onChange={(e) => dispatch({ type: "male-selection", village: e.target.value, gotra: "" })}>
          ...
          {getVillagesForMales().map((village, i) => (
            <option key={i} value={village}>
              {state.user.language ? `${village} (${getMaleMembersWithVillages(village)})` : `${getHindiText(village, "village")} (${getHindiNumbers(getMaleMembersWithVillages(village).toString())})`}
            </option>
          ))}
        </select>
        <label>
          <span>
            {state.user.language ? (
              <>
                <span>{`Married (${getAliveAndMarriedMales()}`}</span>
                <span style={{ fontSize: "8px", color: "red" }}>{`/${getDeadAndMarriedMales()}`}</span>
                {`)`}
              </>
            ) : (
              <>
                <span>{`विवाहित (${getHindiNumbers(getAliveAndMarriedMales().toString())}`}</span>
                <span style={{ fontSize: "8px", color: "red" }}>{`/${getHindiNumbers(getDeadAndMarriedMales().toString())}`}</span>
                {`)`}
              </>
            )}
          </span>{" "}
          <span>
            {state.user.language ? (
              <>
                <span>{`Unmarried (${getAliveAndUnmarriedMales()}`}</span>
                <span style={{ fontSize: "8px", color: "red" }}>{`/${getDeadAndUnmarriedMales()})`}</span>
              </>
            ) : (
              <>
                <span>{`अविवाहित (${getHindiNumbers(getAliveAndUnmarriedMales().toString())}`}</span>
                <span style={{ fontSize: "8px", color: "red" }}>{`/${getHindiNumbers(getDeadAndUnmarriedMales().toString())}`}</span>
                {`)`}
              </>
            )}
          </span>
        ...
        <select name="gotra" value={male.gotra} onChange={(e) => dispatch({ type: "male-selection", village: "", gotra: e.target.value })}>
          ...
          {getGotrasForMales().map((gotra, i) => (
            <option key={i} value={gotra}>
              {state.user.language ? `${gotra} (${getMaleMembersWithGotras(gotra)})` : `${getHindiText(gotra, "gotra")} (${getHindiNumbers(getMaleMembersWithGotras(gotra).toString())})`}
            </option>
          ))}
        </select>
      </div>
      ...
      <div className="filter-women">
        <select name="village" value={female.village} onChange={(e) => dispatch({ type: "female-selection", village: e.target.value, gotra: "" })}>
          ...
          {getVillagesForFemales().map((village, i) => (
            <option key={i} value={village}>
              {state.user.language ? `${village} (${getFemaleMembersWithVillages(village)})` : `${getHindiText(village, "village")} (${getHindiNumbers(getFemaleMembersWithVillages(village).toString())})`}
            </option>
          ))}
        </select>
        <label>
          <span>
            {state.user.language ? (
              <>
                <span>{`Married (${getAliveAndMarriedFemales()}`}</span>
                <span style={{ fontSize: "8px", color: "red" }}>{`/${getDeadAndMarriedFemales()})`}</span>
              </>
            ) : (
              <>
                <span>{`विवाहित (${getHindiNumbers(getAliveAndMarriedFemales().toString())}`}</span>
                <span style={{ fontSize: "8px", color: "red" }}>{`/${getHindiNumbers(getDeadAndMarriedFemales().toString())}`}</span>
                {`)`}
              </>
            )}
          </span>{" "}
          <span>
            {state.user.language ? (
              <>
                <span>{`Unmarried (${getAliveAndUnmarriedFemales()}`}</span>
                <span style={{ fontSize: "8px", color: "red" }}>{`/${getDeadAndUnmarriedFemales()} )`}</span>
              </>
            ) : (
              <>
                <span>{`अविवाहित (${getHindiNumbers(getAliveAndUnmarriedFemales().toString())}`}</span>
                <span style={{ fontSize: "8px", color: "red" }}>{`/${getHindiNumbers(getDeadAndUnmarriedFemales().toString())}`}</span>
                {`)`}
              </>
            )}
          </span>
        ...
        <select name="gotra" value={female.gotra} onChange={(e) => dispatch({ type: "female-selection", village: "", gotra: e.target.value })}>
          ...
          {getGotrasForFemales().map((gotra, i) => (
            <option key={i} value={gotra}>
              {state.user.language ? `${gotra} (${getFemaleMembersWithGotras(gotra)})` : `${getHindiText(gotra, "gotra")} (${getHindiNumbers(getFemaleMembersWithGotras(gotra).toString())})`}
            </option>
          ))}
        </select>
      </div>
