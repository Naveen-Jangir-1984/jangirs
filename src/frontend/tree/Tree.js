            ...
            ? member.wives?.map((wife) => (
                <div className="member-wife-card" key={wife.id}>
                  <img
                    ...
                    src={state.images.find((image) => image.id === wife.id) ? state.images.find((image) => image.id === wife.id).src : FemaleProfileIcon}
                    alt={wife.id}
                    onClick={(e) => {
                      e.stopPropagation();
          ...
          <span className="memberCount">
            {state.user.language ? (
              <span>
                ...
                <span style={{ color: "red", fontSize: "6px" }}>{"/" + countDeadMembers(member)}</span>
              </span>
            ) : (
              <span>
                <span style={{ fontSize: "8px" }}>{getHindiNumbers(countMembers(member).toString())}</span>
                <span style={{ color: "red", fontSize: "6px" }}>{"/" + getHindiNumbers(countDeadMembers(member).toString())}</span>
              </span>
            )}
          </span>
